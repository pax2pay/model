...
const transformers: (authly.Property.Transformer | undefined)[] = [
	...
	new authly.Property.Converter({
		expires: {
			forward: (value: isoly.DateTime) => isoly.DateTime.parse(value).valueOf(),
			backward: (value: number) => isoly.DateTime.create(new Date(value)),
		},
	}),
]
...
export namespace Key {
	...
	export namespace Issuer {
		export function create(type: Type): Issuer {
			// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
			return authly.Issuer.create<Key>(type, authly.Algorithm.create("RS256", keys[type]))!.add(...transformers)
		}
	}
